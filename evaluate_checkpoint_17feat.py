"""Evaluate one 17-feature ("Price+TA+Text") checkpoint on the held-out test split.

Parallel to ``evaluate_checkpoint.py`` (the 6-feature pipeline's evaluator);
identical logic, wired to ``config_17feat.Config17Feature`` and
``src.data_17feat.CMINWindowDataset17Feat`` instead.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate a 17-feature paper-model checkpoint once on CMIN test data.")
    ...
    config = Config17Feature(**saved_config)
    dataset = CMINWindowDataset17Feat(args.dataset_root, args.cache_root, "test", seq_len=config.seq_len)
    ...
    # timeline independently instead of accidentally joining one ticker's
    # last session to another ticker's first session.
    per_stock = []
